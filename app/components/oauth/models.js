...
function revokeAuthorizationCode(code) {
  ...
  return OAuthAuthorizationCode.findOne({
    ...
    expiredCode.expiresAt = new Date('1989-12-29T19:45:53.000Z')
    return expiredCode
  }).catch(function (err) {
    console.log('getUser - Err: ', err)
  })
...
function revokeToken(token) {
  ...
  return OAuthRefreshToken.findOne({
    ...
    expiredToken.refreshTokenExpiresAt = new Date('1989-12-29T19:45:53.000Z')
    return expiredToken
  }).catch(function (err) {
...
function getUserFromClient() {
  return User.findOne({ username: process.env.OAUTH_CLIENT_USER }).then((user) => {
    return user
  })
}
